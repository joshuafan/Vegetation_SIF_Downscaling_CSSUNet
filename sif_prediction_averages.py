...
linear_regression = LinearRegression().fit(X_eval_subtile, Y_eval_subtile) #train, Y_train)  #X_train, Y_train)
...
for idx, crop_type in enumerate(COVER_COLUMN_NAMES):
    ...
    if len(predicted) >= 2:
        ...
        crop_regression = GradientBoostingRegressor().fit(X_crop, Y_crop)
