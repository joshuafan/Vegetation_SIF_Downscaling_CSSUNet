...
UNLABELED_FILES_TRAIN = [os.path.join(PROCESSED_DATASET_DIR, "unlabeled_tiles_" + str(tile_degrees) + "_degrees_train.csv")
                         for tile_degrees in LARGE_TILE_DEGREE_SIZES]
UNLABELED_FILES_TEST = [os.path.join(PROCESSED_DATASET_DIR, "unlabeled_tiles_" + str(tile_degrees) + "_degrees_test.csv")
                        for tile_degrees in LARGE_TILE_DEGREE_SIZES]
FRACTION_TEST = 0.3
...
print('Image metadata columns', list(image_metadata.columns))
unlabeled_tile_info_train = [[list(image_metadata.columns)] for tile_degrees in LARGE_TILE_DEGREE_SIZES]
unlabeled_tile_info_test = [[list(image_metadata.columns)] for tile_degrees in LARGE_TILE_DEGREE_SIZES]

# Loop through all image files
for idx, image_row in image_metadata.iterrows():
    # Load image
    print('image row', image_row)
    image = np.load(image_row['tile_file'])
    split = image_row['split']
    image_center_lat = image_row['lat']
    image_center_lon = image_row['lon']
    image_date = image_row['date']
    image_left_lon, image_top_lat = sif_utils.get_large_grid_area_coordinates(image_center_lon, image_center_lat)
    print('Image file', image_row['tile_file'], 'shape', image.shape)
    ...
    for idx, large_tile_degrees in enumerate(LARGE_TILE_DEGREE_SIZES):
        ...
        print('large tile pixels', LARGE_TILE_PIXELS)
        
        # Get the appropriate output tile folder and metadata list (depending on if this is a train/test tile)
        if split == 'train':
            LARGE_TILE_DIR = os.path.join(DATA_DIR, "large_tiles_" + str(large_tile_degrees) + "_degrees_train")
            tile_info = unlabeled_tile_info_train[idx]
        else:
            LARGE_TILE_DIR = os.path.join(DATA_DIR, "large_tiles_" + str(large_tile_degrees) + "_degrees_test")
        ...
        for top_idx in top_indices:
            for left_idx in left_indices:
                ...
                large_tile_lon = image_left_lon + RES[1] * ((bottom_idx + top_idx) / 2)
                large_tile_lat = image_top_lat - RES[0] * ((left_idx + right_idx) / 2)
                print('Top/left idx', top_idx, left_idx)
                print('Lat/lon', large_tile_lat, large_tile_lon)
                large_tile_filename = os.path.join(LARGE_TILE_DIR, "image_lat_" + str(
                    large_tile_lat) + "_lon_" + str(large_tile_lon) + ".npy")
                ...
                csv_row = [large_tile_lon, large_tile_lat, image_date, large_tile_filename] + average_input_features.tolist()
...
for idx, tile_info in enumerate(unlabeled_tile_info_train):
    with open(UNLABELED_FILES_TRAIN[idx], 'w') as output_csv_file:
        csv_writer = csv.writer(output_csv_file, delimiter=",", quoting=csv.QUOTE_MINIMAL)
        for row in tile_info:
            csv_writer.writerow(row)
for idx, tile_info in enumerate(unlabeled_tile_info_test):
    with open(UNLABELED_FILES_TEST[idx], 'w') as output_csv_file:
        csv_writer = csv.writer(output_csv_file, delimiter=",", quoting=csv.QUOTE_MINIMAL)
        for row in tile_info:
            csv_writer.writerow(row)
