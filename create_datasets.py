...
# Columns for images where we have a SIF label
SIF_TILE_COLUMNS = IMAGE_COLUMNS + ['SIF', 'tropomi_cloud_fraction', 'num_soundings']
...
for DATE_RANGE in DATE_RANGES:
            ...
            for reflectance_file in sorted(os.listdir(REFLECTANCE_DIR)):
                try:
                    with rio.open(os.path.join(REFLECTANCE_DIR, reflectance_file)) as reflectance_dataset:
                        ...
                        print("Rounded bounds: lon:", LEFT_BOUND, "to", RIGHT_BOUND, "lat:", BOTTOM_BOUND, "to", TOP_BOUND)

                        # Iterate through all 0.1 degree intervals
                        num_tiles_lon = int(round((RIGHT_BOUND - LEFT_BOUND) / SIF_TILE_DEGREE_SIZE))
                        num_tiles_lat = int(round((TOP_BOUND - BOTTOM_BOUND) / SIF_TILE_DEGREE_SIZE))
                        ...
                        for left_degrees in tile_lefts:
                            for top_degrees in tile_tops:
                                ...
                                for data_var in reprojected_fldas_dataset.data_vars:
                                    ...
                                    fldas_layers.append(reprojected_fldas_dataset[data_var].data.astype(np.float32))
                                ...
                                assert combined_tile.dtype == np.float32

                                # Compute averages of each band (over non-cloudy pixels)
                                average_input_features = sif_utils.compute_band_averages(combined_tile, combined_tile[MISSING_REFLECTANCE_IDX])
                                reflectance_coverage.append(1 - average_input_features[MISSING_REFLECTANCE_IDX])
                                if average_input_features[MISSING_REFLECTANCE_IDX] > MAX_MISSING_REFLECTANCE:
                                    continue
                                ...
                                csv_row = [center_lon, center_lat, start_date_string, tile_filename] + average_input_features.tolist() + [total_sif, cloud_fraction, num_soundings]
    ...
    plot_histogram(np.array(reflectance_coverage), "reflectance_coverage_" + start_date_string + ".png")
                        ...
                        # ============================ OLD CODE ============================
                        # left_bound_outer = math.floor(LEFT_BOUND)
                        # right_bound_outer = math.ceil(RIGHT_BOUND)
                        # bottom_bound_outer = math.floor(BOTTOM_BOUND)
                        # top_bound_outer = math.ceil(TOP_BOUND)

                        # # Compute "grid lines" between images, 1 degree apart
                        # image_lefts = np.arange(left_bound_outer, right_bound_outer, IMAGE_DEGREE_SIZE)
                        # image_tops = np.arange(top_bound_outer, bottom_bound_outer, -IMAGE_DEGREE_SIZE)
                        # if len(image_lefts) <= 0 or len(image_tops) <= 0:
                        #     print("No overlap between reflectance and cover dataset!")
                        #     continue
                        # print('Image lefts', image_lefts, 'image tops', image_tops)

                        # # # Create large images (1x1 degree)
                        # for image_top in image_tops:
                        #     for image_left in image_lefts:
                        #         image_top_degrees = min(image_top, TOP_BOUND)
                        #         image_left_degrees = max(image_left, LEFT_BOUND)
                        #         image_bottom_degrees = max(image_top - IMAGE_DEGREE_SIZE, BOTTOM_BOUND)
                        #         image_right_degrees = min(image_left + IMAGE_DEGREE_SIZE, RIGHT_BOUND)
                        #         image_center_lon = round((image_left_degrees + image_right_degrees) / 2, 2)
                        #         image_center_lat = round((image_bottom_degrees + image_top_degrees) / 2, 2)
                        #         print('===============================================')
                        #         print('Image degrees: lat', image_bottom_degrees, 'to', image_top_degrees, '; lon', image_left_degrees, 'to', image_right_degrees)

                        #         # Find indices of image in reflectance and cover datasets.
                        #         reflectance_top_idx, reflectance_left_idx = lat_long_to_index(image_top_degrees, image_left_degrees, reflectance_dataset.bounds.top, reflectance_dataset.bounds.left, target_res)
                        #         reflectance_bottom_idx, reflectance_right_idx = lat_long_to_index(image_bottom_degrees, image_right_degrees, reflectance_dataset.bounds.top, reflectance_dataset.bounds.left, target_res)
                        #         cover_top_idx, cover_left_idx = lat_long_to_index(image_top_degrees, image_left_degrees, cover_dataset.bounds.top, cover_dataset.bounds.left, target_res)
                        #         cover_bottom_idx, cover_right_idx = lat_long_to_index(image_bottom_degrees, image_right_degrees, cover_dataset.bounds.top, cover_dataset.bounds.left, target_res)
                        #         print("Reflectance dataset idx: top", reflectance_top_idx, "bottom", reflectance_bottom_idx,
                        #             "left", reflectance_left_idx, "right", reflectance_right_idx)
                        #         print("Cover dataset idx: top", cover_top_idx, "bottom", cover_bottom_idx,
                        #             "left", cover_left_idx, "right", cover_right_idx)
                        #         image_height_pixels = reflectance_bottom_idx - reflectance_top_idx
                        #         image_width_pixels = reflectance_right_idx - reflectance_left_idx
                        #         if image_height_pixels <= 0:
                        #             print('Image height was 0')
                        #             exit(1)
                        #             continue

                        #         if image_width_pixels <= 0:
                        #             print('Image width was 0')
                        #             exit(1)
                        #             continue
                        ...
                        #         # Resample FLDAS data for this tile into target resolution (if we haven't already)
                        #         new_lat = np.linspace(image_top_degrees, image_bottom_degrees, image_height_pixels, endpoint=False)  # bottom_bound, combined_top_bound, height_pixels)
                        #         new_lon = np.linspace(image_left_degrees, image_right_degrees, image_width_pixels, endpoint=False)  # combined_left_bound, combined_right_bound, width_pixels)
                        #         reprojected_fldas_dataset = fldas_dataset.interp(X=new_lon, Y=new_lat)
                        #         fldas_layers = []
                        #         # print('FLDAS data vars', reprojected_fldas_dataset.data_vars)
                        #         for data_var in reprojected_fldas_dataset.data_vars:
                        #             assert(data_var in FLDAS_VARS)
                        #             fldas_layers.append(reprojected_fldas_dataset[data_var].data.astype(np.float32))
                        #         fldas_image = np.stack(fldas_layers)

                        #         # Extract the areas from cover and reflectance datasets that are part of this image
                        #         # Again, index (0, 0) is in the upper-left corner. For "reprojected_covers",
                        #         # the first axis is latitude (higher indices = lower latitudes / south), and the
                        #         # second axis is longitude (higher indices = higher longitudes / east). For 
                        #         # "reprojected_reflectances", it's similar, but there is a "channel" axis first.
                        #         cover_image = reprojected_covers[cover_top_idx:cover_bottom_idx,
                        #                                          cover_left_idx:cover_right_idx]
                        #         reflectance_image = reprojected_reflectances[:, reflectance_top_idx:reflectance_bottom_idx,
                        #                                                      reflectance_left_idx:reflectance_right_idx]
                        #         print('Cover image shape', cover_image.shape, 'dtype', cover_image.dtype)
                        #         print('Reflectance image shape (should be the same!)', reflectance_image.shape, 'dtype', reflectance_image.dtype)
                        #         print('FLDAS image shape (should be the same!)', fldas_image.shape, 'dtype', fldas_image.dtype)

                        #         # Create cover bands (binary masks)
                        #         masks = []
                        #         for cover_type in COVERS_TO_MASK:
                        #             crop_mask = np.zeros_like(cover_image, dtype=bool)
                        #             crop_mask[cover_image == cover_type] = 1.
                        #             masks.append(crop_mask)

                        #         # Also create a binary mask, which is 1 for pixels where reflectance
                        #         # data (for all bands) is missing (due to cloud cover)
                        #         reflectance_sum_bands = reflectance_image.sum(axis=0)
                        #         missing_reflectance_mask = np.zeros_like(reflectance_sum_bands, dtype=bool)
                        #         missing_reflectance_mask[reflectance_sum_bands == 0] = 1.
                        #         masks.append(missing_reflectance_mask)
                        ...
                        #         # Stack reflectance bands and masks on top of each other
                        #         combined_image = np.concatenate((reflectance_image, fldas_image, masks), axis=0)
                        #         print('Combined area shape', combined_image.shape, 'dtype', combined_image.dtype)

                        #         # Write image to .npy file
                        #         image_filename = os.path.join(OUTPUT_IMAGES_DIR, "image_lat_" + str(
                        #             image_center_lat) + "_lon_" + str(image_center_lon) + ".npy")
                        #         np.save(image_filename, combined_image)
                        #         print("Image file", image_filename)

                        #         # Compute image metadata
                        #         average_input_features = sif_utils.compute_band_averages(combined_image, combined_image[MISSING_REFLECTANCE_IDX])
                        #         if average_input_features[MISSING_REFLECTANCE_IDX] > MAX_MISSING_REFLECTANCE:
                        #             continue
                        #         csv_row = [image_center_lon, image_center_lat, start_date_string, image_filename] + average_input_features.tolist()
                        #         image_rows.append(csv_row)



                                # # Iterate through all 0.1 degree intervals
                                # num_tiles_lon = int(round((image_right_degrees - image_left_degrees) / SIF_TILE_DEGREE_SIZE))
                                # num_tiles_lat = int(round((image_top_degrees - image_bottom_degrees) / SIF_TILE_DEGREE_SIZE))
                                # tile_lefts = np.linspace(image_left_degrees, image_right_degrees, num_tiles_lon, endpoint=False)
                                # tile_tops = np.linspace(image_top_degrees, image_bottom_degrees, num_tiles_lat, endpoint=False)
                                # print('Tile lefts', tile_lefts)
                                # print('Tile tops', tile_tops)
                        
                                # # For each "SIF tile", extract the portion of the image that maps to it
                                # for left_degrees in tile_lefts:
                                #     for top_degrees in tile_tops:
                                #         # (this is not needed)
                                #         bottom_degrees = top_degrees - (TARGET_TILE_SIZE * target_res[0])
                                ...
                                #         # Find indices of tile within this image
                                #         image_top_idx, image_left_idx = lat_long_to_index(top_degrees, left_degrees, image_top_degrees, image_left_degrees, target_res)
                                #         image_bottom_idx = image_top_idx + TARGET_TILE_SIZE
                                #         image_right_idx = image_left_idx + TARGET_TILE_SIZE

                                #         # Extract relevant portion of image
                                #         combined_tile = combined_image[:, image_top_idx:image_bottom_idx, image_left_idx:image_right_idx]

                                #         # If tile contains NaNs, skip it
                                #         if np.isnan(combined_tile).any():
                                ...
                                #         # # Remove tiles where no pixels have data (completely covered by clouds)
                                #         # if np.mean(combined_tile[MISSING_REFLECTANCE_IDX]) > 0.99:
                                #         #     continue
                                ...
                                #         # print('===========================================')
                                #         # print('Avg input features (calc from compute_band_averages)', average_input_features)
                                #         # print('Avg input features (manual)', tile_averages)
                                #         # print('===========================================')
                                ...
                                #         # Add metadata about the tile to csv
                                #         csv_row = [center_lon, center_lat, start_date_string, tile_filename] + average_input_features.tolist() + [total_sif, cloud_fraction, num_soundings, image_filename]
                                #         dataset_rows.append(csv_row)
    ...
    # # Write information about each image to the output csv file
    # with open(OUTPUT_IMAGES_CSV_FILE, mode) as output_csv_file:
    #     csv_writer = csv.writer(output_csv_file, delimiter=",", quoting=csv.QUOTE_MINIMAL)
