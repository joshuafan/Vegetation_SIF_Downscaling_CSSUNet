...
# CFIS filtering
eps = 1e-5
MIN_EVAL_CFIS_SOUNDINGS = 30
MIN_EVAL_FRACTION_VALID = 0.9-eps
MIN_EVAL_FRACTION_VALID_EXPERIMENT = [0.9-eps]
MIN_SIF_CLIP = 0.1
MAX_SIF_CLIP = None
...
def compare_unet_to_others(args, model, dataloader, device, sif_mean, sif_std, resolution_meters, MIN_EVAL_CFIS_SOUNDINGS, min_eval_fraction_valid,
    ...
    for sample in dataloader:
        with torch.set_grad_enabled(False):
            ...
            if type(outputs) == tuple:
                outputs = outputs[0]
            predicted_fine_sifs_std = outputs[:, 0, :, :]
            predicted_fine_sifs = predicted_fine_sifs_std * sif_std + sif_mean
            ...
            for i in range(input_tiles_std.shape[0]):
                ...
                # Mask for plotting purposes only
                plot_mask = (eval_fraction_valid[i] >= min_eval_fraction_valid).cpu().detach().numpy()

                true_coarse_sif_tile = true_coarse_sifs[i].cpu().detach().numpy()
                true_eval_sifs_tile = true_eval_sifs[i].cpu().detach().numpy()
                predicted_eval_sifs_linear = np.zeros(non_noisy_mask_tile.shape)
                predicted_eval_sifs_mlp = np.zeros(non_noisy_mask_tile.shape)
                ...
                # Get (pre-computed) averages of valid coarse/fine regions, to plot results under different methods
                ...
                if args.plot_examples and sample['fraction_valid'][i] > 0.5:
                    print('Plotting')
                    # Plot example tile
                    true_eval_sifs_tile[plot_mask == 0] = 0
                    predicted_eval_sifs_linear[plot_mask == 0] = 0
                    predicted_eval_sifs_mlp[plot_mask == 0] = 0
                    predicted_eval_sifs_unet[plot_mask == 0] = 0
...
def main():
    ...
    for RESOLUTION_METERS in RESOLUTIONS:
        CFIS_EVAL_METADATA_FILE = os.path.join(DATA_DIR, 'cfis_fine_metadata_' + str(RESOLUTION_METERS) + 'm.csv')
