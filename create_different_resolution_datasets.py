...
RES_AVERAGE_COLUMNS = ['lon', 'lat', 'date', 'tile_file', 'ref_1', 'ref_2', 'ref_3', 'ref_4',
...
FINE_PIXELS_PER_COARSE = [20, 10, 5, 3, 1]
...
for resolution_pixels in FINE_PIXELS_PER_COARSE:
    ...
    for phase, cfis_metadata in cfis_metadatas.items():
        res_metadata = []
        RES_AVERAGES_FILE = os.path.join(CFIS_DIR, 'cfis_averages_' + resolution_meters + 'm_' + phase + '.csv')
        for idx, row in cfis_metadata.iterrows():
            ...
            valid_sif_mask = torch.logical_not(torch.tensor(fine_sifs_numpy.mask))
            ...
            for i in range(res_sifs.shape[0]):
                for j in range(res_sifs.shape[1]):
                    ...
                    average_input_features = sif_utils.compute_band_averages(input_subregion, input_subregion[MISSING_REFLECTANCE_IDX])
                    ...
                    subregion_row = [subregion_lon, subregion_lat, row['date'], row['tile_file']] + average_input_features.tolist() + \
