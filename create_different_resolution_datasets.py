...
RES_AVERAGE_COLUMNS = ['fold', 'lon', 'lat', 'date', 'tile_file', 'ref_1', 'ref_2', 'ref_3', 'ref_4',
    ...
    for idx, row in cfis_metadata.iterrows():
        ...
        for i in range(res_sifs.shape[0]):
            for j in range(res_sifs.shape[1]):
                ...
                subregion_row = [row['fold'], subregion_lon, subregion_lat, row['date'], row['tile_file']] + average_input_features.tolist() + \
