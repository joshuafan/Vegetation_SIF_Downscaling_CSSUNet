...
parser.add_argument('-method', "--method", choices=["Ridge_Regression", "Gradient_Boosting_Regressor", "MLP", "Random_Forest", "Nearest_Neighbors"], type=str, help='Method type. MLP is the fully-connected artificial neural netwoprk.')
...
parser.add_argument('-log', "--log", action='store_true', help='Whether to log the reflectance features.')
...
MIN_FINE_CFIS_SOUNDINGS = [30]  #[1, 5, 10, 20, 30] # # 100, 250] #[100, 300, 1000, 3000]
eps = 1e-5
...
if args.match_coarse:
    CFIS_TRUE_VS_PREDICTED_PLOT += "_matchcoarse"
    args.method += "_matchcoarse"

RESULTS_SUMMARY_FILE = os.path.join(RESULTS_DIR, "results_summary_BASELINE_SOUNDINGS.csv")
...
for min_coarse_fraction_valid in MIN_COARSE_FRACTION_VALID_PIXELS:
    ...
    for min_fine_cfis_soundings in MIN_FINE_CFIS_SOUNDINGS:
        for min_fraction_valid_pixels in MIN_FINE_FRACTION_VALID_PIXELS:
            results_rows = {s: [args.method, s, min_fine_cfis_soundings, min_fraction_valid_pixels, args.mult_noise_std] for s in TRAIN_RANDOM_STATES}
            ...
            for resolution in RESOLUTION_METERS:
                ...
                cfis_fine_metadata = cfis_fine_metadata[(cfis_fine_metadata['SIF'] >= MIN_SIF_CLIP) &
                                                        ...
                                                        (cfis_fine_metadata['fraction_valid'] >= 0.5)]
                ...
                cfis_fine_metadata_filtered = cfis_fine_metadata[(cfis_fine_metadata['num_soundings'] >= min_fine_cfis_soundings) &
                                                        (cfis_fine_metadata['fraction_valid'] >= min_fraction_valid_pixels)]  # Avoid roundoff errors
                ...
                # Create filtered versions of fine datasets (with less noisy pixels)
                fine_train_filter_mask = (fine_train_set['num_soundings'] >= min_fine_cfis_soundings) & (fine_train_set['fraction_valid'] >= min_fraction_valid_pixels)
                fine_val_filter_mask = (fine_val_set['num_soundings'] >= min_fine_cfis_soundings) & (fine_val_set['fraction_valid'] >= min_fraction_valid_pixels)
                fine_test_filter_mask = (fine_test_set['num_soundings'] >= min_fine_cfis_soundings) & (fine_test_set['fraction_valid'] >= min_fraction_valid_pixels)
                fine_train_set_filtered = fine_train_set[fine_train_filter_mask]
                fine_val_set_filtered = fine_val_set[fine_val_filter_mask]
                fine_test_set_filtered = fine_test_set[fine_test_filter_mask]
                X_fine_train_filtered = X_fine_train[fine_train_filter_mask]
                Y_fine_train_filtered = Y_fine_train[fine_train_filter_mask]
                X_fine_val_filtered = X_fine_val[fine_val_filter_mask]
                Y_fine_val_filtered = Y_fine_val[fine_val_filter_mask]
                X_fine_test_filtered = X_fine_test[fine_test_filter_mask]
                Y_fine_test_filtered = Y_fine_test[fine_test_filter_mask]
                ...
                if 'Linear_Regression' in args.method:
                    ...
                    n_estimators_values = [10, 30, 100, 300, 1000]
                    max_features_values = [2, 5, None]
                    for n_estimators in n_estimators_values:
                        for max_features in max_features_values:
                            models = []
                            for random_state in TRAIN_RANDOM_STATES:
                                regression_model = RandomForestRegressor(n_estimators=n_estimators, max_features=max_features, random_state=random_state).fit(X_train, Y_train)
                                models.append(regression_model)
                            param_string = 'n_estimators=' + str(n_estimators) + ', max_features=' + str(max_features)
                            print(param_string)
                            regression_models[param_string] = models             
                else:
                ...
                for params, models in regression_models.items():
                    ...
                    for model in models:  # Loop through all model runs (trained with different seeds)
                        predictions_val_filtered = model.predict(X_fine_val_filtered)
                        loss_val = math.sqrt(mean_squared_error(Y_fine_val_filtered, predictions_val_filtered)) / sif_mean
                ...
                for idx, model in enumerate(regression_models[best_params]):
                    ...
                    # print("Predictions fine train before all this", predictions_fine_train_filtered)
                    ...
                    if args.match_coarse:
                        fine_train_set["predicted_sif"] = predictions_fine_train_all
                        fine_train_set["predicted_coarse_sif"] = fine_train_set.groupby('tile_file')['predicted_sif'].transform('mean')
                        fine_train_set["predicted_sif"] *= (fine_train_set["coarse_sif"] / fine_train_set["predicted_coarse_sif"])
                        # print("After adjustment", fine_train_set_filtered[["tile_file", "SIF", "predicted_sif", "coarse_sif", "predicted_coarse_sif"]].head(10))
                        predictions_fine_train_filtered = fine_train_set["predicted_sif"][fine_train_filter_mask].to_numpy()
