...
TRAINING_PLOT_FILE = 'loss_plots/losses_' + METHOD + '.png'
...
def train_model(model, dataloaders, criterion, optimizer, device, sif_mean, sif_std, num_epochs=25):
        ...
        for phase in ['train', 'val']:
            ...
            for sample in dataloaders[phase]:
                if 'tropomi_tile' in sample:
                    ...
                    tropomi_tiles_std = sample['tropomi_tile'].to(device)  # (batch size, num channels, H, W)
    ...
    return model, train_tropomi_losses, train_oco2_losses, val_tropomi_losses, val_oco2_losses, train_tropomi_mse, train_oco2_mse, val_tropomi_mse, val_oco2_mse, best_oco2_loss
...
if 'TROPOMI' in TRAIN_SOURCES:
    train_tropomi_metadata = train_metadata[train_metadata['source'] == 'TROPOMI'].iloc[0:10000]
...
transform_list_train = [] #noise_transform] # [standardize_transform, noise_transform]
...
model, train_tropomi_losses, train_oco2_losses, val_tropomi_losses, val_oco2_losses, train_tropomi_mse, train_oco2_mse, val_tropomi_mse, val_oco2_mse, best_loss = train_model(model, dataloaders, criterion, optimizer, device, sif_mean, sif_std, num_epochs=NUM_EPOCHS)
...
print('============== Val OCO2 vs TROPOMI losses ===============')
sif_utils.print_stats(val_oco2_losses, val_tropomi_losses, sif_mean)
plt.scatter(val_tropomi_losses, val_oco2_losses)
plt.xlabel('Val TROPOMI losses')
plt.ylabel('Val OCO2 losses')
plt.title('Val: OCO2 vs TROPOMI losses' + METHOD)
plt.xlim(left=0, right=0.5)
plt.ylim(bottom=0, top=0.5)
plt.savefig(TRAINING_PLOT_FILE + '_val_oco2_vs_tropomi.png')
plt.close()
